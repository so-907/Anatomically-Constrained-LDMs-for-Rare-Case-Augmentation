...
geometrically consistent with the tooth root. Built to answer a question the reference paper leaves
open, and evaluated against explicit measurement floors and ceilings.
...
risks losing "precise spatial guidance", and explicitly name the missing experiment: *"we did not
compare to ControlNet-like latent diffusion models trained completely from scratch."* This project
builds exactly that, plus a pixel-space baseline through identical code, and an anatomical
...
(249.9 → 253.9 latent; 139.8 → 138.0 pixel), and it regularises: both
...
seen), the best model realises **38% of the commanded change**; the worst, 14%. The dataset contains
**3 Severe teeth out of 575**. Neither architecture nor loss closes that.

A fourth, methodological finding: **the constrained pixel model exceeds the real-image ceiling** on
mask conformance (0.9001 vs 0.8127) with FID unchanged: generated images are *easier to segment* than
real radiographs. Dice-against-mask stops measuring realism well before a generator stops improving
it, which is why every table here reports FID/KID beside it.
...
There are only 3 Severe teeth, and it is why
the headline result is about what cannot be done rather than what can.
...
```
Anatomically_Constrained_LDMs_for_Rare_Case_Augmentation.ipynb   the entire project, in the seven mandated sections
PLAN.md                                                          the plan approved before implementation, kept unedited for comparison
figs/                                                             every figure in this README, generated by the notebook
cache/                                                            checkpoints and precomputed artefacts (not committed, ~860 MB)
perio_KPT/                                                        the dataset (not committed, see above)
Computer_Vision_Presentation/                                     LaTeX beamer source for the slides
```
...
Verified on a single RTX 5060 Laptop GPU, 8151 MiB VRAM, 14 GB RAM. The 8 GB ceiling is the
binding constraint on every design decision in the project (patch resolution, VAE capacity, batch
sizes, run lengths).
...
delete the checkpoint to force a rerun. From a cold cache the full pipeline is roughly:
...
[![Forward diffusion process on one training patch at t=0, 100, 300, 600, 999: the tooth is fully legible at t=100, mostly gone by t=600, pure noise at t=999](figs/a2_forward_process.png)](figs/a2_forward_process.png)
*The forward process on a training patch. All models here (latent and pixel) predict the noise added at a sampled `t`; the anatomical loss only reads the model's `x0` estimate, so it is restricted to low `t` (see below).*

### Patch definition

Tooth boxes are median 354 × 1093 px (≈1:3). A square resize would destroy the aspect ratio the whole
project depends on, so instead:

1. Rotate about the box centre so the tooth long axis is **vertical**; all anatomical lines then become
   near-horizontal. The sign of the rotation was resolved empirically over all 575 teeth.
2. **Canonicalise crown-up.** Periapical radiographs contain maxillary (crown down) and mandibular
   (crown up) teeth and the rotating-box angle does not say which; the CEJ/apex keypoints do. Removing
   this 2-way ambiguity roughly doubles the effective dataset. 206 of 575 patches are flipped.
3. Crop the box widened to aspect 1:2 with 15% margin, deliberately pulling in the neighbouring
   alveolar bone, which the model needs in order to render a plausible crest.
4. Resize to **256 × 128**, grayscale, normalise to [−1, 1].

[![Three-step patch pipeline: (0) rotating boxes and keypoints on the raw radiograph, (1) eight canonicalised crown-up patches with their PBL and flip flag, (2) six finished 5-class masks colour-coded blue/red/green/gold across Moderate and Severe teeth](figs/steps1-3.png)](figs/steps1-3.png)
*Steps 0–2 of the pipeline: box + keypoints on the raw image, canonicalised crown-up patches with PBL and flip flag, and the finished 5-class mask overlay.*

### The anatomical mask (the conditioning signal)

A 5-class label map built per patch in the rotated, crown-up frame:
...
*Worked example: keypoints → PBL → 5-class mask, for a Severe tooth.*
...
*`S_build`'s training data: auxiliary tooth patches (top) and the image reconstructed from nothing but their binary mask (bottom) — a sanity check that the mask alone carries most of the tooth's shape.*
...
*`S_build` silhouettes across the PBL range (val Dice 0.9221); this is the mask that everything else in the project is built on top of.*

This encodes severity **geometrically**: `PBL = (y_crest − y_CEJ) / (y_apex − y_CEJ)`. Commanding a
severe case is therefore a matter of translating the crest line apically (`retarget_mask`), which
requires no severe training example — that is the augmentation engine the project set out to build,
and testing whether it actually works is result 3 above.
...
*The 5-class mask across all four severity bands. The two PBL numbers on each panel — read from the keypoints and read back from the mask — are the redundancy check behind Gate 3.*
...
*Close-up on the two boundary lines (CEJ, crest) that actually separate the three tooth-region classes — the geometry the conditioning signal has to get right.*
...
*Three independent PBL estimates:`S_build`'s own silhouette, `S_eval` reading that silhouette back, and the keypoint ground truth, agreeing to within a few points. This is what makes it safe to treat the constructed masks as ground truth downstream.*


### Conditioning

Following Konz §1.2, the mask is concatenated channel-wise to the network input at every denoising
step. Our latent version nearest-downsamples the integer label map to 64×32 and **one-hots to 5
channels**. One-hot rather than a single integer channel, because integer labels acquire false
ordinality under downsampling.

**Mask-ablated training** (Konz Algorithm 1): per sample, per class, Bernoulli erasure. We use
`p_class = 0.25`, not Konz's 0.5, because 393 training patches is not thousands, and the change
